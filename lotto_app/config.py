...
def parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument("--xlsx", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "data.xlsx"), help="Path to the output workbook.")
    ...
    parser.add_argument("--sample-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "sample_features.csv"), help="Path to the exported sample feature table.")
    parser.add_argument("--rule-report-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "rule_effectiveness.csv"), help="Path to the exported rule effectiveness report.")
    parser.add_argument("--rule-grid-report-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "rule_grid_report.csv"), help="Path to the exported multi-config rule comparison report.")
    parser.add_argument("--rule-grid-summary-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "rule_grid_summary.csv"), help="Path to the exported summarized multi-config rule ranking report.")
    parser.add_argument("--model-ranking-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "model_ranking.csv"), help="Path to the exported latest-draw model ranking table.")
    parser.add_argument("--model-metrics-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "model_metrics.csv"), help="Path to the exported model evaluation metrics.")
    parser.add_argument("--blue-model-ranking-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "model_blue_ranking.csv"), help="Path to the exported latest-draw blue-ball model ranking table.")
    parser.add_argument("--blue-model-metrics-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "model_blue_metrics.csv"), help="Path to the exported blue-ball model evaluation metrics.")
    parser.add_argument("--candidate-pools-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "candidate_pools.csv"), help="Path to the exported candidate pool summary.")
    parser.add_argument("--candidate-combinations-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "candidate_combinations.csv"), help="Path to the exported constrained red-ball candidate combinations.")
    parser.add_argument("--strategy-backtest-csv", default=str(Path(__file__).resolve().parents[1] / "data" / "output" / "strategy_backtest.csv"), help="Path to the exported historical strategy backtest report.")
